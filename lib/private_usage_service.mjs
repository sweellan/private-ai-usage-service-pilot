...
export function buildSparseLabelIndices(total, maxLabels = 5) {
...
<html lang="zh-CN">
  ...
  <body>
    ...
    <script>
      ...
      function buildSparseLabelIndices(total, maxLabels = 5) {
        if (!Number.isFinite(total) || total <= 0) return [];
        if (total <= maxLabels) return Array.from({ length: total }, (_, index) => index);
        const anchors = [
          0,
      ...
      function renderStackedChart(scope, rows) {
        ...
        if (scope.kind === 'team') {
          ...
          const labelIndices = new Set(buildSparseLabelIndices(rankingRows.length, 5));
          ...
          bars = rows.map((row, index) => {
            ...
            const barGroup = series.map((member, memberIndex) => {
              ...
              const label = row.period === latestPeriod && value > 0 && labelIndices.has(memberIndex)
