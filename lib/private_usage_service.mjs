...
<html lang="zh-CN">
  ...
  <body>
    ...
    <script>
      ...
      function resetRangeToRecent(rows) {
        if (!rows.length) {
        ...
        uiState.startIndex = Math.max(0, rows.length - 30);
      }
      ...
      function renderStackedChart(scope, rows) {
        ...
        const title = scope.kind === 'team' ? '团队当前周期排名' : '按时间总量 + Breakdown';
        ...
        const height = scope.kind === 'team'
          ? Math.max(360, 130 + ((teamRanking?.rows.length || 0) * 34))
          : 340;
        const left = 72, right = 28, top = 56, bottom = 92;
        const plotWidth = width - left - right;
        const plotHeight = height - top - bottom;
        const maxValue = Math.max(...rows.map((row) => Number(row.total || row.accountedTotalTokens || 0)), 1);
        ...
        if (scope.kind === 'team') {
          ...
          const rankingMax = Math.max(...rankingRows.map((row) => Number(row.currentTokens || 0)), 1);
          const rowHeight = Math.max(28, Math.min(40, plotHeight / Math.max(rankingRows.length, 1)));
          const labelWidth = 170;
          const valueWidth = 96;
          const barAreaWidth = plotWidth - labelWidth - valueWidth - 18;
          bars = rankingRows.map((row, index) => {
            const y = top + 12 + index * rowHeight;
            const barY = y - 12;
            const barHeight = Math.max(16, rowHeight - 10);
            const barWidthPx = rankingMax > 0 ? Math.max(6, (Number(row.currentTokens || 0) / rankingMax) * barAreaWidth) : 0;
            const barX = left + labelWidth;
            const labelX = left + labelWidth - 10;
            const valueX = barX + barWidthPx + 10;
            const fill = colorPalette[index % colorPalette.length];
            const valueLabel = compact.format(Number(row.currentTokens || 0));
            return '' +
              '<text x="' + labelX + '" y="' + y + '" text-anchor="end" font-size="12" font-weight="600" fill="#12344d">' + escHtml(row.memberName) + '</text>' +
              '<rect x="' + barX + '" y="' + barY + '" width="' + barAreaWidth + '" height="' + barHeight + '" fill="#eaf1f6" rx="8"></rect>' +
              '<rect data-index="' + index + '" x="' + barX + '" y="' + barY + '" width="' + barWidthPx + '" height="' + barHeight + '" fill="' + fill + '" rx="8"></rect>' +
              '<text x="' + Math.min(valueX, width - right - valueWidth) + '" y="' + y + '" font-size="12" font-weight="700" fill="#12344d">' + escHtml(valueLabel) + '</text>';
          }).join('');
          legend = '<text x="' + left + '" y="' + (height - 26) + '" font-size="12" fill="#476072">All members sorted by current period</text>';
        } else {
